...
class RegisterUi:
    def __init__(self, root, user_service: UserService, on_back, settings_service):
        ...
        # Titlu modernizat
        tk.Label(self.container, text="Creează un cont nou", font=("Segoe UI", 22, "bold"),
        ...
        # Câmpuri (folosind culorile dinamice)
        ...
        tk.Button(self.container, text="Înapoi", command=on_back, bg=self.colors["bg"],
    ...
    def handle_register(self):
        # ... (Logica de extragere date rămâne neschimbată) ...
        ...
        if password != confirm_pass:
            self.show_toast("❌ Parolele nu coincid!", "#E74C3C")
            return

        if not all([username, password, data_path]):
            self.show_toast("⚠️ Toate câmpurile sunt obligatorii!", "#F1C40F")
            return
        ...
        try:
            ...
            if result and result[0] == 201:
                # Nu închidem imediat pentru a lăsa userul să vadă mesajul de succes
                self.show_toast("✅ Cont creat! Te poți loga.")
