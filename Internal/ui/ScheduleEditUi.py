...
class ScheduleEditUi(tk.Toplevel):
    def __init__(self, parent, theme, cell_id, day, current_data, on_save, on_delete, group_service,
        ...
        group_grades = [g.get_grade() for g in self.available_groups]
        values = []
        for i in range(0, len(group_names)):
            values.append(group_grades[i] + "." + group_names[i])

        self.group_combo = ttk.Combobox(self, values=values, font=("Segoe UI", 11), state="readonly")
